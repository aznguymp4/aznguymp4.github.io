		...
		function reform(d) {
			...
			Object.keys(d).map(pl => {
				d[pl].scores.map(score => {
					delete d[pl].scores
					idk.push(Object.assign(score, d[pl]))
				})
			})
			return idk
		}
		...
		function buildTable(data,useLocalIDX){
			...
			data.sort(scoreSort).map((player, i) => {
				...
				`<tr>
					...
					<td class="username" title="Click to Copy User ID" data-id="${player.id}"><img src="${player.avatar == 'N/A'? `https://cdn.discordapp.com/embed/avatars/${Math.floor(Math.random() * 5)}.png` : `${player.avatar.length==5? 'https://cdn.discordapp.com/embed/avatars' : `https://cdn.discordapp.com/avatars/${player.id}`}/${player.avatar}`}?size=32" height="32" width="32">${player.username.replace(/.{5}$/,'')}</td>
